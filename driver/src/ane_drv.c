...
static int ane_info(struct drm_device *drm, void *data,
                    struct drm_file *file)
{
    struct ane_device *ane = drm->dev_private;
    struct drm_ane_info *args = data;
...
static int ane_submit(struct drm_device *drm, void *data, struct drm_file *file)
{
	...
	for (int bdx = 0; bdx < ane->hw->bar_count; bdx++) {
		if (args->handles[bdx]) {
			...
			if (!bo || !bo->iova ||
				...
				return -EINVAL;
			if (ane->hw->id <= DRM_ANE_ID_H13 && upper_32_bits(bo->iova))
				return -EINVAL;
			req.bar[bdx] = bo->iova;
		}
	}
	...
	if (ane->hw->id <= DRM_ANE_ID_H13 && upper_32_bits(bo->iova))
		return -EINVAL;
	req.btsp_iova = bo->iova;

	mutex_lock(&ane->engine_lock);

	err = ane->hw->tm_enqueue(ane, &req);
	if (err < 0)
		goto unlock;

	err = ane->hw->tm_execute(ane, &req);
	if (err < 0)
		goto unlock;
...
static const struct drm_ioctl_desc ane_drm_ioctls[] = {
	DRM_IOCTL_DEF_DRV(ANE_INFO, ane_info, 0),
...
static int ane_platform_probe(struct platform_device *pdev)
{
	...
	ane->engine = devm_platform_get_and_ioremap_resource(pdev, 0, NULL);
...
static const struct ane_hw ane_hw_h13 = {
	.dart = {
		.vm_base = 0x4000,
		.vm_size = 0xe0000000,
	},
	.id = DRM_ANE_ID_H13,
	.bar_count = 0x20,
	.tm_enable = ane_h13_tm_enable,
	.tm_enqueue = ane_h13_tm_enqueue,
	.tm_execute = ane_h13_tm_execute,
};

static const struct ane_hw ane_hw_h14 = {
	.dart = {
		.vm_base = 0x4000,
		.vm_size = 0xF00000000ULL, // TODO: I'm making this up but H14 has 64-bit BARs so probably larger vm_size
	},
	.id = DRM_ANE_ID_H14,
	.bar_count = 0x61,
	.tm_enable = ane_h14_tm_enable,
	.tm_enqueue = ane_h14_tm_enqueue,
	.tm_execute = ane_h14_tm_execute,
};

static const struct of_device_id ane_of_match[] = {
	{ .compatible = "apple,t8103-ane", .data = &ane_hw_h13 },
	{ .compatible = "apple,t6000-ane", .data = &ane_hw_h13 },
    { .compatible = "apple,t8112-ane", .data = &ane_hw_h14 },
	{}
};
